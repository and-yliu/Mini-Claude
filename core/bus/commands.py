...
from core.session.model import SessionMode, SessionStatus
...
class EventSubscribeCommand(BaseModel):
    ...
    topics: list[str]          # fnmatch patterns, e.g., ["step.*", "tool.*"]
    scope: str = "global"      # "global" | "run:<run_id>"
    replay_from_run: str | None = None  # if set, replay history from events.jsonl before real-time stream
...
# Command discriminated by type field
Command = Annotated[
    PingCommand
    | AgentRunCommand
